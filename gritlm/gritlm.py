...
class GritLMTextModel(
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            h = layer(layer_idx_t, h, kv_collection, input_row_offsets=input_row_offsets)
        ...
        if self.return_logits == ReturnLogits.VARIABLE:
            return_n_logits_range = ops.range(
                return_n_logits[0], 0, -1,
                out_dim="return_n_logits_range",
                device=h.device, dtype=DType.int64,
            )
            offset_tensor = F.unsqueeze(input_row_offsets[1:], -1) - return_n_logits_range
            last_indices = F.reshape(offset_tensor, shape=(-1,))
            logits = self._compute_logits(self.norm(F.gather(h, last_indices, axis=0)))
            offsets = ops.range(
                0,
...
class GritLM(Module[..., tuple[Tensor, ...]]):
    ...
    def __init__(self, config: GritLMConfig, kv_params: KVCacheParamInterface) -> None:
        super().__init__()
