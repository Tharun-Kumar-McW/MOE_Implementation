# ===----------------------------------------------------------------------=== #
# GritLM model configuration for MAX (ModuleV3 API).
#
# GritLM-7B is Mistral-7B with:
#   - GQA: 32 Q heads / 8 KV heads
#   - SwiGLU MLP, RMSNorm, no bias
#   - RoPE theta=10000 (standard Mistral, not LLaMA-3)
#   - Sliding window attention: sliding_window=4096 on ALL layers
#   - tie_word_embeddings: false (separate lm_head)
#   - vocab_size: 32000 (Mistral/Llama-1 tokenizer)
#   - max_position_embeddings: 32768
#
# GritLM adds a pooling head for embeddings but we only implement the
# CausalLM path for MAX text generation serving.
# ===----------------------------------------------------------------------=== #
...
class GritLMConfig(ArchConfigWithKVCache):
    ...
    def calculate_attention_multiplier(huggingface_config: AutoConfig) -> float:
        return getattr(
            ...
            math.sqrt(1.0 / float(GritLMConfig.get_head_dim(huggingface_config))),
        )
    ...
    def initialize(
        ...
        sliding_window: int = getattr(
            huggingface_config, "sliding_window", 4096
        ) or 4096  # guard against None
    ...
    def finalize(
        ...
        normalized = (
            {_strip(k, "model."): v for k, v in state_dict.items() if k.startswith("model.")}
            if has_model else dict(state_dict)
        )
        ...
        self.rms_norm_eps = (
            huggingface_config.rms_norm_eps if norm_method == "rms_norm" else None
        )
